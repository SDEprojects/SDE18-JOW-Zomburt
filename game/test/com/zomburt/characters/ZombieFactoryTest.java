...
public class ZombieFactoryTest {
    ...
    public void testCreateZombie_HARD_positive() {
        ...
        int zombieInitialHealth = zombie2.getHealth()-zombie2.getInventory().get(0).getDamage();
        assertTrue(Arrays.stream(ZombieTypes.values()).map(e -> e.getName()).anyMatch(zombie2.getName()::equals));
        assertTrue(Arrays.stream(ZombieTypes.values()).map(e -> Integer.toString(e.getHealth())).anyMatch(Integer.toString(zombieInitialHealth)::equals));
